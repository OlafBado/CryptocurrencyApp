    ...
    return (
        <header ref={isNavOpen && width < 700 ? ref : null}>
            <div className="container navbar__wrapper">
                <Link to='/'>
                    <img onClick={() => width < 700 ? setIsNavOpen(false) : null} src={logo} alt="navigation logo" className='logo'/>
                </Link>
                <nav className='navbar' style={{display: isNavOpen ? 'block' : 'none'}}>
                    <ul className='navbar__list'>
                        <NavLink to="/" className={({isActive}) => (isActive ? 'navbar__list__link--active' : 'navbar__list__link')}>
                            <li onClick={handleNavbar} className='navbar__list__item'>
                                Home
                            </li>
                        </NavLink>
                        <NavLink to="/cryptocurrencies" className={({isActive}) => (isActive ? 'navbar__list__link--active' : 'navbar__list__link')}>
                            <li onClick={handleNavbar} className='navbar__list__item'>
                                Cryptocurrencies
                            </li>
                        </NavLink>
